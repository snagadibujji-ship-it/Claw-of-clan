...
- 本 Skill 集成自 `GHIA-Ecosystem/secknowledge-skill`，在 GHIA Scout 中作为 `secknowledge-skill` specialized skill 使用；上游声明为 MIT License。
- CTF/SRC 场景先加载 `references/ghia-scout-ctf-src-routing.md` 判断资料入口，再按漏洞类型加载 `web-*`、`ai-*`、`testing-methodology.md` 或 `gaarm-risk-matrix.md`。
- 与 GHIA Scout 现有技能协同：CTF 单题技巧优先结合 `ctf-web`/`ctf-crypto`/`ctf-misc`，SRC 和实战漏洞挖掘优先使用本 Skill 的方法论、案例映射、风险矩阵和证据约束。
