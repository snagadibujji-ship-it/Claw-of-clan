...
async function requestJson<T>(input: string, init?: RequestInit): Promise<T> {
  ...
  try {
    ...
    throw new Error("Unable to reach the GHIA Scout backend API. Start `ghia-scout web` and reconnect.");
  }
  ...
  try {
    ...
    throw new Error("The backend API returned non-JSON content. Confirm the backend was started with `ghia-scout web`.");
  }
}
