...
from ghia_scout.config.token_provider import has_llm_credentials
from ghia_scout.i18n import _
from ghia_scout.orchestrator import run_agent_task
...
def _run_repl() -> None:
    ...
    while True:
        try:
            ...
            prompt_str = " | ".join(prompt_parts) if prompt_parts else "ghia_scout"

            # Read input
            user_input = console.input(f"ghia_scout {prompt_str}> ").strip()
            ...
            if cmd_lower in ("exit", "quit", "q"):
                ...
                from ghia_scout.agent.core import PersistentCycleResult
...
def _generate_report_for_target(
    ...
    *,
    ...
    from ghia_scout.report.generator import generate_report, generate_report_from_target_state
    from ghia_scout.target_state.store import load_target_state
...
def report(
    ...
    if target_mode:
        from ghia_scout.report.generator import generate_report_from_target_state
        ...
        from ghia_scout.report.generator import generate_report_from_file
...
def config_provider(
    ...
    # Switch provider
    from ghia_scout.config.schema import PROVIDER_PRESETS, LLMProvider
...
def init() -> None:
    ...
    from ghia_scout.config.settings import ensure_dirs
...
def login(
    ...
    console.print("  Run [bold]ghia_scout[/] to start.")


@app.command()
def logout() -> None:
    """Remove stored OAuth tokens (browser sign-in)."""
    from ghia_scout.config.token_provider import logout_oauth
...
def doctor() -> None:
    ...
    console.print()
    if has_key:
        console.print("[green]Environment ready. Run [bold]ghia_scout[/] to start.[/]")
    else:
        console.print(
...
def plugins_list(
    ...
    from ghia_scout.plugins import registry
...
def plugins_run(
    ...
    from ghia_scout.plugins import PluginContext, PluginStage, create_builtin_runtime
...
def kb_status() -> None:
    ...
    console.print(
        Panel(
            ...
            f"语义搜索: 运行 [bold]pip install ghia_scout\\[kb][/] 启用 ChromaDB",
            title="KB Status",
            border_style="cyan",
        )
    )
...
def _auto_save_recon_report(agent, user_input: str, config) -> None:
    ...
    try:
        ...
        from ghia_scout.report.generator import generate_report
...
def tui(
    ...
    if once and target:
        from ghia_scout.cli.tui import render_tui_home
...
def web(
    ...
    from ghia_scout.web.app import FASTAPI_AVAILABLE
    ...
    if not FASTAPI_AVAILABLE:
        err_console.print(
            "[!] FastAPI is missing. Install with [bold]pip install ghia_scout[web][/]."
        )
        raise typer.Exit(1)
    ...
    try:
        ...
        err_console.print(
            "[!] uvicorn is missing. Install with [bold]pip install ghia_scout[web][/]."
        )
        raise typer.Exit(1)

    from ghia_scout.web.app import create_app
