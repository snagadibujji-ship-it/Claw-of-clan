"""FastAPI app entry for the GHIA Scout Web UI backend."""

from __future__ import annotations

from pathlib import Path

from ghia_scout.web.schemas import ConfigUpdateRequest, ReportGenerateRequest, TaskCreateRequest
from ghia_scout.web.services.config_service import get_public_config, update_public_config
...
from ghia_scout.web.services.report_service import (
    generate_target_report,
    list_reports,
    read_report_content,
    resolve_report_path,
)
from ghia_scout.web.services.target_service import (
    clear_target,
    get_diff,
    get_preview,
    get_snapshots,
...
from ghia_scout.web.services.task_service import start_task
...
def create_app():
    ...
    if not FASTAPI_AVAILABLE:
        raise RuntimeError(
            "FastAPI is not installed. Install the web extra first: pip install ghia_scout[web]"
        )

    app = FastAPI(title="GHIA Scout Web UI", version="0.3.2")

    @app.get("/api/health")
    async def health():
        return {"status": "ok", "service": "ghia_scout-web"}
