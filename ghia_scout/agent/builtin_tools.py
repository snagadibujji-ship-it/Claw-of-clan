...
from ghia_scout.agent.constraint_policy import validate_tool_action
...
BLOCKED_PATTERNS: list[str] = [
    ...
    r"shutil\.\s*rmtree\s*\(",
    r"__import__\s*\(\s*['\"]os['\"]",
    r"open\s*\(\s*['\"].*ghia_scout.*config",
    r"open\s*\(\s*['\"].*\.ghia_scout",
]
...
async def execute_mcp_tool(agent: Any, tool_name: str, args: dict[str, Any]) -> str:
    ...
    if constraints is not None:
        ...
        if tool_violation is not None:
            if session is not None and hasattr(session, "add_constraint_violation_event"):
                from ghia_scout.agent.constraint_policy import infer_tool_action
    ...
    if tool_name == "load_skill_reference":
        try:
            from ghia_scout.skills.loader import load_skill_reference
    ...
    if tool_name == "crypto_decode":
        try:
            from ghia_scout.skills.crypto_tools import execute as crypto_execute
    ...
    from ghia_scout.agent import advanced_exploit_tools
    ...
    from ghia_scout.agent import web_exploit_tools
    ...
    from ghia_scout.agent import ctf_binary_tools
    ...
    from ghia_scout.agent.session_store import dispatch_session_tool
    ...
    from ghia_scout.agent import osint_tools
    ...
    from ghia_scout.agent import payload_generator
    ...
    from ghia_scout.agent import auto_chain
    ...
    from ghia_scout.agent import cloud_misconfig
...
def _write_python_audit(
    ...
    *,
    ...
    try:
        ...
        from ghia_scout.config.settings import PYTHON_EXECUTE_AUDIT_FILE, ensure_dirs
