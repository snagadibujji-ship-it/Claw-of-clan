...
<br>
项目官网：https://unclecheng-li.github.io/ghia-scout.com/
<br>
...
- **漏洞检测插件体系** — 低耦合插件运行时 + 内置只读 Web 插件，结果自动汇入报告链路（`ghia-scout plugins`）
...
- **Web UI 模式** — `ghia-scout web` 启动本地 Web 界面，浏览器操作渗透测试全流程，默认 `127.0.0.1:7788`
...
循环结构（`ghia-scout/agent/solver.py`）：
...
低耦合插件运行时（`ghia-scout/plugins/`）+ 内置只读 Web 插件（安全响应头 / JWT / JS 端点分析），插件结果可去重合并进 `SessionState.findings` 进入报告链路。

> 切回旧的固定轮数引擎：`ghia-scout config set session.engine rounds`
...
```bash
cp .env.example .env          # 填入 GHIA_SCOUT_LLM_API_KEY 等
...
ghia-scout config provider minimax   (或 openai/deepseek/zhipu/moonshot/qwen/siliconflow)
...
#      ghia-scout login   （浏览器登录；详见 docs/keyless-auth.md，注意 ToS 风险）
...
| 命令 | 说明 | 示例 |
|------|------|------|
| `ghia-scout` | 默认打开原 CLI / REPL 交互界面 | `ghia-scout` |
| `ghia-scout tui` | 显式打开终端图形化工作台 | `ghia-scout tui` / `ghia-scout tui --target target.com` |
| `ghia-scout repl` | 启动经典 REPL 交互界面 | `ghia-scout repl` |
| `ghia-scout solve <target>` | 目标驱动求解（无固定轮数，拿到目标即停） | `ghia-scout solve target.com --goal "拿到flag"` |
| `ghia-scout run <target>` | 一键全流程渗透（默认走 solve 引擎） | `ghia-scout run 192.168.1.1` |
| `ghia-scout persistent <target>` | 持续性渗透（100轮/周期） | `ghia-scout persistent 192.168.1.1` |
| `ghia-scout recon <target>` | 仅信息收集（不利用漏洞） | `ghia-scout recon target.com` |
| `ghia-scout scan <target>` | 漏洞扫描阶段 | `ghia-scout scan target.com --ports 80,443` |
| `ghia-scout exploit <target>` | 漏洞利用阶段 | `ghia-scout exploit target.com --cve CVE-2024-1234` |
| `ghia-scout report <session>` | 从会话 JSON 生成报告 | `ghia-scout report session_xxx.json` |
| `ghia-scout config set <key> <value>` | 设置配置项 | `ghia-scout config set llm.api_key sk-xxx` |
| `ghia-scout config get <key>` | 查看配置项 | `ghia-scout config get llm.model` |
| `ghia-scout config list` | 列出所有配置 | `ghia-scout config list` |
| `ghia-scout config provider <name>` | 切换 LLM 提供商 | `ghia-scout config provider minimax` |
| `ghia-scout init` | 初始化配置文件 | `ghia-scout init` |
| `ghia-scout doctor` | 检查运行环境 | `ghia-scout doctor` |
| `ghia-scout plugins list` | 列出漏洞检测插件 | `ghia-scout plugins list --stage discovery` |
| `ghia-scout plugins info <id>` | 查看插件元信息 | `ghia-scout plugins info builtin.web.headers` |
| `ghia-scout plugins run <id>` | 运行插件（仅分析传入数据） | `ghia-scout plugins run builtin.web.headers --input headers.json --session s.json` |
| `ghia-scout web` | 启动本地 Web UI | `ghia-scout web` / `ghia-scout web --port 8080` |

### TUI 工作台

`ghia-scout tui` 是可选的终端图形化工作台入口。它会在终端中展示授权目标、检查模式、运行概览、安全边界、命令预览、历史状态、报告和内联环境诊断，让用户先确认范围再启动任务。
...
默认 `ghia-scout` 仍然进入原 CLI / REPL 交互；只有显式输入 `ghia-scout tui` 才会进入 TUI。
...
```bash
# 查看所有提供商并切换
ghia-scout config provider --list    # 查看所有可用提供商
ghia-scout config provider minimax   # 切换到 MiniMax
...
```
🦞 ghia-scout> 对 192.168.1.100 进行渗透测试，这是我授权的靶场
...
菜单 7 “环境诊断入口”会在 TUI 内显示 Python、Node/npx/uvx/nmap、LLM 配置和 MCP 服务/工具摘要；需要完整详情时再运行 `ghia-scout doctor`。
...
| `clear`               | 清空当前会话                               |
| `help`                | 显示帮助信息                               |
| `exit` / `quit` / `q` | 退出 GHIA Scout                              |

#### 自主渗透模式

GHIA Scout 检测到以下关键词 + 目标时，自动进入多轮自主渗透循环：
...
适用于需要长时间深度渗透的场景。GHIA Scout 以**周期循环**方式运行：
...
```bash
# CLI 方式
ghia-scout persistent 192.168.1.100              # 默认 100轮/周期 × 10周期
ghia-scout persistent 192.168.1.100 -r 200 -c 5  # 200轮/周期 × 5周期
ghia-scout persistent 192.168.1.100 --no-report   # 不自动生成报告
...
GHIA Scout 支持所有 OpenAI 兼容协议的 API，内置 8 个提供商预设：

```bash
ghia-scout config provider --list    # 查看所有提供商
ghia-scout config provider minimax   # 一键切换
```
...
# Step 2: 启用 GHIA Scout 配置（自动通过 npx 拉取，无需手动安装）
ghia-scout config set mcp.servers.chrome-devtools.enabled true
```

GHIA Scout 配置已内置 `npx -y chrome-devtools-mcp@latest`，启用后自动连接。如需指定 Chrome 调试地址，编辑 `~/.ghia-scout/config.yaml`：
...
# Step 4: 启用 GHIA Scout 配置
ghia-scout config set mcp.servers.burp.enabled true
```
  ...
  servers:
    burp:
      ...
      transport:
        type: stdio
        command: java
        args: ["-jar", "~/.ghia-scout/tools/burp-mcp-all.jar", "--sse-url", "http://127.0.0.1:9876"]
```
...
`secknowledge-skill` 集成自 [`GHIA-Ecosystem/secknowledge-skill`](https://github.com/GHIA-Ecosystem/secknowledge-skill)，上游 `references/` 的 38 个文档已完整纳入，并额外增加 `ghia-scout-ctf-src-routing.md` 作为 GHIA Scout 的 CTF/SRC 场景导航。它会在 `SRC`、`漏洞挖掘`、`众测`、`GAARM`、`OWASP LLM/ASI/WSTG`、`Web+AI` 等强信号输入下触发，用于按需加载 SQLi、XSS、RCE、SSRF、AI/MCP、Agent、风险矩阵和测试方法论等资料。
...
```bash
ghia-scout config list                          # 查看所有配置
ghia-scout config get llm.model                 # 查看单项
ghia-scout config set llm.api_key sk-xx         # 设置 API Key
ghia-scout config set session.max_rounds 30     # 设置自主渗透最大轮数（默认 15）
ghia-scout config set session.stale_rounds_threshold 8  # 设置死循环检测阈值（默认 5）
ghia-scout config set session.show_thinking false # 隐藏推理过程（也可在 REPL 中用 think off）
```
...
| `llm.auth_mode`          | static | `static`（api_key）或 `oauth`（`ghia-scout login`） |
...
| `session.output_dir`     | ./ghia-scout-output | 报告输出目录                    |
...
- **新增目标驱动求解引擎（默认）** — 基于 Fact/Intent 黑板图的 OODA 循环，以「目标达成 / 探索前沿耗尽 / 安全预算」为终止条件，结构上杜绝"原地打转"；新增 `ghia-scout solve` 命令，`run`/REPL 自主模式默认改走该引擎（`session.engine=rounds` 可回退旧逻辑）。
...
- **新增漏洞检测插件体系** — 低耦合插件运行时 + 内置只读 Web 插件（安全响应头 / JWT / JS 端点），结果可去重合并进 findings 与报告链路；新增 `ghia-scout plugins list/info/run` 命令。
...
GHIA Scout 仅用于**已授权的安全测试**。使用本工具前，请确保：
...
| ![GHIA Scout 社区交流群](assets/社区交流群.jpg) | ![GHIA Scout 开发者群聊](assets/GHIA Scout开发者群聊.png) |
